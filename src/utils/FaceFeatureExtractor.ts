...
export class FaceFeatureExtractor {
   ...
   * 모든 얼굴 특징을 계산합니다.
   */
  private static calculateAllFeatures(points: Array<{ x: number; y: number }>): FaceFeatures {
    return {
   ...
   * 얼굴형 특징 계산 (4차원)
   */
  private static calculateFaceShapeFeatures(points: Array<{ x: number; y: number }>): FaceFeatures['faceShape'] {
    // 얼굴 경계점들 (얼굴 윤곽선)
    ...
    // 얼굴 가로세로 비율
    const faceWidth = Math.max(...faceOutline.map(p => p.x)) - Math.min(...faceOutline.map(p => p.x));
    const faceHeight = Math.max(...faceOutline.map(p => p.y)) - Math.min(...faceOutline.map(p => p.y));
   ...
   * 눈 특징 계산 (4차원)
   */
  private static calculateEyeFeatures(points: Array<{ x: number; y: number }>): FaceFeatures['eyeFeatures'] {
    // 왼쪽 눈 (36-41)
   ...
   * 입 특징 계산 (3차원)
   */
  private static calculateMouthFeatures(points: Array<{ x: number; y: number }>): FaceFeatures['mouthFeatures'] {
    // 입 (48-67)
    ...
    // 입 높이
    const mouthHeight = Math.max(...mouth.map(p => p.y)) - Math.min(...mouth.map(p => p.y));
   ...
   * 코 특징 계산 (2차원)
   */
  private static calculateNoseFeatures(points: Array<{ x: number; y: number }>): FaceFeatures['noseFeatures'] {
    // 코 (27-35)
    const nose = points.slice(27, 36);
    
    // 코 길이
    const noseLength = Math.max(...nose.map(p => p.y)) - Math.min(...nose.map(p => p.y));
    
    // 코 너비 (코 끝 부분의 너비)
    const noseTip = points.slice(31, 36);
    const noseWidth = Math.max(...noseTip.map(p => p.x)) - Math.min(...noseTip.map(p => p.x));
   ...
   * 얼굴 비율 계산 (2차원)
   */
  private static calculateFaceProportions(points: Array<{ x: number; y: number }>): FaceFeatures['faceProportions'] {
  ...
  private static calculateJawAngle(jawPoints: Array<{ x: number; y: number }>): number {
  ...
  private static calculateFaceSymmetry(points: Array<{ x: number; y: number }>): number {
    // 얼굴 중앙선 기준으로 대칭성 계산
    const faceCenterX = (Math.min(...points.map(p => p.x)) + Math.max(...points.map(p => p.x))) / 2;
    ...
    const eyeSymmetry = 1 - Math.abs((leftEye.x - faceCenterX) + (rightEye.x - faceCenterX)) / 100;
    const noseSymmetry = 1 - Math.abs((leftNostril.x - faceCenterX) + (rightNostril.x - faceCenterX)) / 50;
    
    return (eyeSymmetry + noseSymmetry) / 2;
  }

  private static calculateEyeSize(eyePoints: Array<{ x: number; y: number }>): number {
  ...
  private static getEyeCenter(eyePoints: Array<{ x: number; y: number }>): { x: number; y: number } {
  ...
  private static calculateEyeAngle(eyePoints: Array<{ x: number; y: number }>): number {
  ...
  private static calculateLipThickness(lipPoints: Array<{ x: number; y: number }>): number {
