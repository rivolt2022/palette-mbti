...
// 색상 팔레트 생성기
class DiverseColorPaletteGenerator {
  /**
  ...
  private static generateContrastPalette() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const lightness = i % 2 === 0 ? 0.2 + Math.random() * 0.3 : 0.6 + Math.random() * 0.3;
...
class FaceFeatureGenerator {
  ...
  static generateFaceDescriptors(count: number = 1000): Array<{
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 얼굴 특징 분석
      const characteristics = this.analyzeFaceCharacteristics(descriptor, physicalFeatures);
   ...
   * 얼굴 특징 분석
   */
  private static analyzeFaceCharacteristics(descriptor: number[], physicalFeatures: number[]): {
    faceShape: string;
    eyeSize: string;
    mouthSize: string;
    noseSize: string;
    ...
    return {
      faceShape: faceRatio > 0.7 ? 'round' : faceRatio < 0.4 ? 'long' : 'oval',
      eyeSize: eyeSize > 0.7 ? 'large' : eyeSize < 0.3 ? 'small' : 'medium',
      mouthSize: mouthWidth > 0.7 ? 'wide' : mouthWidth < 0.3 ? 'narrow' : 'medium',
      noseSize: noseWidth > 0.7 ? 'wide' : noseWidth < 0.3 ? 'narrow' : 'medium',
      jawline: jawAngle > 0.7 ? 'angular' : jawAngle < 0.3 ? 'soft' : 'moderate'
    };
  }
...
function generateDiverseFaceColorDataset(sampleCount: number = 10000) {
  ...
  const colorPalettes = DiverseColorPaletteGenerator.generateDiversePalettes(sampleCount);
  
  // 2. 다양한 얼굴 특징 생성
  console.log('👤 다양한 얼굴 특징 생성 중...');
  const faceFeatures = FaceFeatureGenerator.generateFaceDescriptors(sampleCount);
...
// 메인 실행 함수
async function main() {
  const OUTPUT_DIR = join(process.cwd(), 'public', 'data', 'diverse-face-color');
  ...
  try {
    ...
    Object.entries(colorCategories).forEach(([category, count]) => {
      console.log(`  ${category}: ${count}개 (${(count / dataset.length * 100).toFixed(1)}%)`);
    });
